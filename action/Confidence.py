...
class Confidence(Action):
    NAME = "置信水平评估"
    DESC = "根据问题和知识库检索的匹配程度，评估当前智能体的置信水平，和回答问题的智能体并发执行"
    PROMPT_TEMPLATE = (Base_DIR / 'prompt/ConfidencePrompt.txt').read_text(encoding="utf-8")
    PARSER = JSONExtractor()

    def __init__(self, **kwargs):
        super().__init__(name=self.NAME, desc=self.DESC, **kwargs)

    def build_prompt(self, info: str, question: str, reasoning: bool = REASONING) -> "Confidence":
        """
        构建prompt
